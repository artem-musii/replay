...
_Actual 1280 × 720 capture from the exact configured-base release payload in the Codex in-app browser. It shows the current Scene calibration UI, 1.25× playback, 12-point paths, 18 registered Site Tools, the live baseline beside a pending review-only proposal, and separate case/session activity. The proposal call was operator-directed; this is not a supported-model-choice claim._
...
An operator also opened this exact configured-base payload in the Codex in-app browser and invoked four page-defined Site Tools through its native bridge: `get_case_summary`, `get_workspace_state` for scene/questions, all-scope `validate_case_consistency`, and `focus_workspace_item` for `question-lane-change`. All returned `ok: true` at case v1; validation returned the single `integrity.calibration-source` question, focus visibly opened that question, and the activity panel showed one durable seed change plus four session-only calls, each labelled **No case change · observed v1**. Browser runtime logs contained no warning or error. This is exact-release-payload native bridge execution and visible UI/session-audit agreement—not supported-model choice, **Recently used/Sources**, mutation/lifecycle, or broad cross-client evidence.

A post-deploy in-app-browser smoke independently loaded the public URL with no warning/error logs, discovered the 18 baseline Site Tools, and invoked summary, structured state, all-scope validation, and recent activity against a fresh synthetic case. It also reproduced the one-time impact pause at 10.0 s, advanced to 13.66 s and remained playing after one Play click, verified both roundabout vehicles changed position and diverged from 62°/62° at impact to 42°/25° afterward, and opened the labelled 65/77 km/h high-speed scenario. These are operator-directed live product and bridge checks, not an uncoached supported-model trace.
