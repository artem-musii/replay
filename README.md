...
**Current deployed application payload:** application commit [`b2e93905ff349a29f21b0b544a59e3afc738671d`](https://github.com/artem-musii/replay-sol/commit/b2e93905ff349a29f21b0b544a59e3afc738671d) passed [GitHub Actions run `33272807674`](https://github.com/artem-musii/replay-sol/actions/runs/33272807674), including verify/build job `99154232692`, deploy job `99155253861`, and post-deploy byte-verification job `99155282304`: **460/460 Vitest tests across 37 files**, **230 Playwright project runs: 221 passed, 9 intentional mobile screenshot-owner skips, and 0 failed**, plus a **12/12** configured-base release matrix. Pages deployment `6159696063` published artifact `9720702224` (3,655,213 compressed bytes; SHA-256 `17357bb96e0ac622b190377e81a415df47704c9b7a42dc78763b1aa0293b7fbe`). The retained release-evidence artifact is `9720702014` (SHA-256 `08190cf5f735812766331a5fd4c7110e919515c9a7756962f23991d5a65640ab`). The live [`release-evidence.json`](https://artem-musii.github.io/replay-sol/release-evidence.json) identifies the latest clean wrapper commit and attests the same **46 public payload files / 5,297,260 bytes** with manifest SHA-256 `586c81a32c8b0d15deed08ecd99ebd069697a2158aa0ca047d87cdd0f0e6bb87`; hosted CI and independent post-deploy verification fetch and match every file. Documentation/test-only follow-up commits may therefore change the wrapper commit recorded by that endpoint without changing the attested application payload.
...
A post-deploy in-app-browser smoke of the preceding `cd88755b` payload independently exercised its read/UI bridge and remains historical to those bytes. A separate cache-busted operator smoke of the current `b2e93905` deployment selected the exact 10.000 s impact marker and advanced to 17.7 s after one Play click; from 9.5 s it auto-paused once at 10.0 s, then advanced to 15.9 s and remained playing after one resume. The public technical inspector visibly exposed `propose_scene_changes` with `changes.minItems=1`, full `trajectory-set` start/final semantics, and separate `mark_impact_event` semantics. There were no console errors or failed dynamic requests; an ordinary Playwright browser emitted only the expected unsupported origin-trial `Permissions-Policy` warnings because `document.modelContext` was absent. This is current live product and published-contract evidence, not native Site Tools invocation or supported-model choice.
