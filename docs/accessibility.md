...
The current source passed **232 Playwright project runs: 223 passed, 9 intentional mobile screenshot-owner skips, and 0 failed**, including automated axe, focus/reflow, exact-editor, guide/tour, iframe-guard, and checked visual-baseline regressions. The live [`release-evidence.json`](https://artem-musii.github.io/replay/release-evidence.json) establishes the exact public release identity after CI. These are automated accessibility guardrails, not a manual assistive-technology or complete WCAG conformance claim.
