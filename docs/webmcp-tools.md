...
Status: **current-source contract; deployment and supported-model execution pending**, last reconciled on 2026-08-28 with the 2026-08-26 WebMCP draft. The inventory remains 19 imperative tools plus the declarative final-review form: 18 before report preview and 19 after `add_report_note` joins. Current source adds calibrated geometry/motion/integrity scopes and exact-revision claim-attestation invalidation without changing that count. Those additions have deterministic source coverage but have not yet been deployed or exercised by a supported Site Tools model.

Historical release evidence remains bounded to application commit `00688d8a51fb783dbf147e08ece60470b8877544`: its standards-compatible deterministic registry verified the then-current 18→19 lifecycle, and a Codex in-app-browser smoke surfaced all 18 baseline registrations without invoking them. A prior `df599f3` deployed-bundle audit used a non-native registry to verify durable observation persistence across a cache-busted navigation, transient-preview clearing, and reset; the preserved `f980d28` native 17/18-tool smoke is also historical. None of these is a supported-model execution trace for the current source. OpenAI's current Site Tools browser exposes top-level imperative JavaScript tools, not declarative HTML form tools. Ordinary browser interaction with a form is not a WebMCP call or authorization to operate REPLAY's human confirmation controls.
...
| `upsert_scene_actor`           | Add/update pose plus declared dimensions in metres, vehicle class, dimension-source label, optional wheelbase, label, and colour. | Scene exists.                                    | Write                      | `false`; `true`                                                      | Vehicle appears/moves on SVG canvas; selection and activity update.                                                                                                | `engine.stage("actor.upsert")`                                               | Agent source must be `template`, `estimated`, or `unknown`; invalid/locked actor; stale/duplicate request; abort.                                                               |
| `set_actor_trajectory`         | Replace a branch-scoped actor trajectory with ordered normalized timed poses.                                                     | Scene and referenced actor/branch exist.         | Write                      | `false`; `true`                                                      | Two poses render linearly; 3+ use the shared timed Hermite curve; playback, swept checks, activity, and issues refresh.                                            | `engine.stage("trajectory.set")`                                             | Missing actor/branch; unordered/out-of-range keyframes; locked trajectory; stale/duplicate request; cancellation before primary save or staged commit.                          |
