...
Application commit `cd88755b9b72e2e0a360a8a877584dc36c7c2053` completed the following matrix on **2026-08-29** in successful [GitHub Actions run `33269347192`](https://github.com/artem-musii/replay-sol/actions/runs/33269347192). The configured-base artifact was then deployed and independently byte-verified.
...
| Production artifacts    | Root: **46 public payload files / 5,295,476 bytes**, plus `.nojekyll`, manifest SHA-256 `4f91a8a4ee753e15ee8fb63d9f14736612dc91fc1f7318b8641685062aa9dd9d`; `/replay-sol/`: **46 / 5,295,872 bytes**, plus `.nojekyll`, SHA-256 `70323dbd1cd355dd3415a242e6c58a361d8617e35dd84a5cf3b1bc161b8e4e5c`                                                                                                                                                                                                                         |
| Diff integrity          | `git diff --check` passed                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Clean-tree release gate | Passed in hosted CI against exact clean commit `cd88755b9b72e2e0a360a8a877584dc36c7c2053`                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Deployment jobs         | Verify/build `99144899873`; deploy `99145725110`; verify-deployment `99145744458`, all successful                                                                                                                                                                                                                                                                                                                                                                                                                          |
| Pages publication       | Deployment `6159028555`; artifact `9719700894`, **3,656,260 compressed bytes**, SHA-256 `1488827c7ee3164568c418eeba147d742be2b6dab9f175a7c0bfa2ecf5b7b875`                                                                                                                                                                                                                                                                                                                                                                 |
| Live payload            | **46 public files / 5,295,872 bytes**, byte-matched by the post-deploy verifier; manifest SHA-256 `70323dbd1cd355dd3415a242e6c58a361d8617e35dd84a5cf3b1bc161b8e4e5c`                                                                                                                                                                                                                                                                                                                                                       |
...
The public URL serves application commit `cd88755b9b72e2e0a360a8a877584dc36c7c2053`. Record the exact browser/client/model and confirm native `document.modelContext` before calling any result Site Tools evidence.
...
**Exact-release-artifact native bridge smoke (2026-08-29):** the 46-payload `/replay-sol/` artifact at 5,295,872 bytes with manifest SHA-256 `70323dbd1cd355dd3415a242e6c58a361d8617e35dd84a5cf3b1bc161b8e4e5c` exposed 18 page-defined Site Tools in the Codex in-app browser before those exact bytes were deployed. An operator invoked `get_case_summary`, `get_workspace_state` for scene/questions, all-scope `validate_case_consistency`, and `focus_workspace_item` for `question-lane-change`. All four returned `ok: true` at v1; validation returned exactly one issue, `integrity.calibration-source`; focus visibly opened the blocking question; and the activity panel showed one durable seed change plus four **Session only** entries, each labelled **No case change · observed v1**. Browser runtime logs had no warning or error. A direct UI playback check on the same artifact paused once at 10.0 s, then advanced to 12.9 s and kept playing after one Play click. This proves native bridge execution and visible UI/session-audit agreement for the deployed artifact bytes. It does not prove a live-deployment invocation, inspect a main-world `document.modelContext` constructor, exercise mutation/lifecycle behavior, establish supported-model choice, capture **Recently used/Sources**, or establish cross-client behavior.

**Current public product and operator-bridge smoke (2026-08-29):** a cache-busted fresh in-app-browser tab loaded the deployed 5,295,872-byte payload with no warning/error logs and surfaced the 18 baseline Site Tools. Operator-directed `get_case_summary`, structured scene/questions/report state, all-scope validation, and recent activity calls returned `ok: true` at case v1 and remained session-only. The roundabout event-click path advanced from exactly 10.000 s to 10.427 s after one Play click; the 9.500 s path auto-paused exactly once at 10.000 s and advanced after one resume click. Vehicle headings diverged from 62°/62° at impact to 42°/25° at the authored final positions, and the separate high-speed case visibly labelled its approximately 65/77 km/h paths as reconstruction values rather than measured speeds. This is live product/bridge evidence for the deployed bytes, not uncoached supported-model choice, **Recently used/Sources**, mutation/lifecycle, or broad client compatibility.
