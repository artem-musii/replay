...
- Public YouTube demo, 2:21 with English audio: [https://www.youtube.com/watch?v=0INcRPRIR04](https://www.youtube.com/watch?v=0INcRPRIR04)
...
The public video resolves through YouTube's public embed metadata, runs for 2:21, includes English narration, and shows the same human-gated WebMCP workflow available at the final live URL. Keep the live URL, repository, testing instructions, and gallery images aligned with that release.
...
The verified public deployment is the complete seed-v6 release containing calibrated geometry/motion/integrity review, physically coherent authored contact geometry, impact/playback polish, four deterministic scenarios, explicit review-only start-to-final WebMCP reconstruction, and the trust, persistence, accessibility, reporting, and release-verification work described below. The live `release-evidence.json` is authoritative for the exact clean wrapper commit and payload manifest. No aggregate supported-model pass rate is claimed, and uncaptured manual gates must not be presented as complete. The public 2:21 YouTube demo is now published.
...
**Deployed application-payload evidence:** current source passed 463 Vitest tests across 38 files, 232 Playwright project runs (223 passed, 9 intentional mobile screenshot-owner skips, 0 failed), and the 12/12 configured-base matrix. The live [`release-evidence.json`](https://artem-musii.github.io/replay/release-evidence.json) identifies the exact clean source commit and payload manifest; hosted CI builds that commit and the post-deploy verifier fetches every declared public file and matches every byte.
...
The Node 22.13.0 `/replay/` artifact contains 46 public payload files / 5,322,811 bytes, plus the deployment-control `.nojekyll`. That exact already-built configured-base artifact passed 12/12 focused runs: release/high-speed/impact 8/8, handler contract 2/2, and submission story 2/2 on desktop and mobile. The deterministic handler used the E2E imperative `document.modelContext` polyfill, registered 18 lifecycle-eligible tools without churn, and returned all eight requested workspace sections, including `selection: null`, below the 32,768-byte compact target and 524,288-byte hard cap. Registry/adapter coverage proves that a complete start-to-final proposal remains preview-only until human acceptance; the submission story proves lifecycle behavior, canonical-geometry-safe proposal/rejection, impact response, provenance-safe agent inference, human confirmation/finalization, and PDF export. The public manifest supplies the final deployment digest.
