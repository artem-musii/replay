...
A screenshot can show two vehicles, but it cannot reliably tell an agent which timed point belongs to which branch, whether a statement is photo-backed or disputed, whether a path is locked, or which case version is current. WebMCP exposes those exact live semantics and validated actions. The agent can run REPLAY's deterministic checks, focus an issue in the interface, create a reversible multi-actor proposal, preserve an inference as an inference, and prepare a cited report from the same domain command layer as the UI—never an agent-only shadow copy.
...
**Agent: read, validate, propose, draft. Reviewer: attest, decide, finalize.** An agent proposal is a visible preview, not a silent geometry change. The UI lets a reviewer compare, adjust, accept, or reject it; a later UI correction becomes newer attributed history that the agent can reread. The agent may report a completeness gap, but no Site Tool can attest to the reviewed no-evidence, damage, or uncertainty state, confirm a claim, or finalize the report. Those commands require the visible UI review origin.
...
- Open-source license: [MIT](https://github.com/artem-musii/replay-sol/blob/main/LICENSE)
- Build and testing instructions: [README](https://github.com/artem-musii/replay-sol#local-setup)
...
- Thumbnail: `docs/images/replay-devpost-thumbnail.jpg`—a real current-candidate 1280 × 720 workspace capture under 5 MB. Replace it only if the final deployed UI differs.
...
The verified public deployment is commit `2855f0bc50da2916128b2278a46f0d0a8a4e2bbd`, a seed-v5 release containing calibrated geometry/motion/integrity review, physically coherent authored contact geometry, and four deterministic scenarios. Its clean CI, Pages artifact, and independent 2026-08-29 artifact-to-cache-busted-public-file comparison are recorded below. Later seed-v6 working-tree fixes remain separately scoped local evidence. The exact dirty candidate now has operator-directed native in-app-browser bridge execution evidence, but no exact-current-deployment supported-model result is claimed. Replace the YouTube placeholder after recording the final public demo, and do not imply deployment or completion of any uncaptured manual gate.
...
- preview-only coordinated agent scene proposals that Site Tools cannot adjust, accept, or reject; and
...
**Deployed-release evidence:** `2855f0bc50da2916128b2278a46f0d0a8a4e2bbd` passed Actions run `33184281134`, including verify job `98893121004` and deploy job `98894240126`, with 196/196 Vitest tests across 20 files and 114 Playwright project runs: 109 passed, 5 intentional mobile screenshot-owner skips, and 0 failed. Pages deployment `6143728209` published artifact `9691136611` (3,032,328 compressed bytes; SHA-256 `27c2bf89662de9280ddca52f9d2cb922545a913a27f819b855110f184e924da9`). An independent 2026-08-29 post-deploy comparison downloaded that artifact and cache-busted all 43 public files / 4,248,606 bytes; every byte matched, yielding manifest SHA-256 `6eb13acc1eec75d60298a3979009a175e2ba94bc8e9ad00382d4a274bdcc6ba4`. That release predates the new automated `release-evidence.json` and `verify-deployment` gate, so the live evidence endpoint is absent.

**Superseded public-browser evidence:** the earlier seed-v3 commit `00688d8a51fb783dbf147e08ece60470b8877544` retains its public guide/scene-editing/recovery smoke, native discovery of 18 baseline tools without invocation, and cache-busted Lighthouse 13.4.1 score of 100/100/100/100. Those results remain historical evidence for that exact release and are not attributed to `2855f0bc`, the later seed-v6 working tree, or any supported-model run.

**Current-source local evidence:** the settled dirty candidate passed 457/457 Vitest tests across 37 files on Node 22.13.0 and 230 Playwright project runs with 221 passed, 9 intentional mobile screenshot-owner skips, and 0 failed, plus 20 checked visual baselines. The browser split was 114/114 desktop Chromium, 105 passed plus 9 skipped in mobile Chrome, and 1/1 release smoke in both Firefox and WebKit. V8 coverage was 63.81% statements (6,765/10,601), 54.21% branches (4,479/8,261), 61.89% functions (1,624/2,624), and 65.88% lines (6,262/9,504); statement coverage was 84.99% in the domain layer, including 99.46% in `demoScenarios.ts` and 95.47% in physics, plus 84.68% in integration and 89.78% in WebMCP. Formatting, zero-warning lint, strict typecheck, both dependency audits with 0 vulnerabilities, dependency-tree resolution, root and `/replay-sol/` production builds on the declared runtime, and `git diff --check` passed.

The Node 22.13.0 root artifact contains 46 public payload files / 5,295,427 bytes, plus the deployment-control `.nojekyll`, with manifest SHA-256 `0544048ef3d96a43683393dd5f3be4d2fdd8f169739334a18636ee1d6d833025`. The `/replay-sol/` artifact contains 46 public payload files / 5,295,823 bytes, plus `.nojekyll`, with manifest SHA-256 `1b4063e0d1a78f691ed78729db5004c017f2f1ae20d7cc2677845e943d7ba9b5`. That exact already-built configured-base artifact passed 12/12 focused runs: release/high-speed/impact 8/8, handler contract 2/2, and submission story 2/2 on desktop and mobile. The deterministic handler used the E2E imperative `document.modelContext` polyfill, registered 18 lifecycle-eligible tools without churn, and returned all eight requested workspace sections, including `selection: null`, in a complete 18,970-byte case-v1 response, below the 32,768-byte compact target and 524,288-byte hard cap. The submission story proved the 18→19→18 lifecycle, canonical-geometry-safe proposal/rejection, impact response, provenance-safe agent inference, human confirmation/finalization, and PDF export.

The exact configured-base artifact also passed an operator-directed native in-app-browser Site Tools trace. `get_case_summary`, `get_workspace_state` for scene/questions, all-scope validation, and focus for `question-lane-change` all returned `ok: true` at v1. Validation returned the single `integrity.calibration-source` question; focus visibly opened it; REPLAY showed one durable seed change plus four session-only calls labelled **No case change · observed v1**; and browser runtime logs had no warning or error. This proves exact-artifact native bridge execution and UI/session-audit agreement, not clean-commit CI, deployment, supported-model choice, **Recently used/Sources**, mutation/lifecycle, or cross-client behavior.

Lighthouse 13.4.1 with Chrome 151 completed three warning-free runs per profile against the exact configured-base artifact. The mobile runs scored 89/91/90 performance, all with 100 accessibility / 100 best practices / 100 SEO, for median performance 90 and medians of FCP 2.032 s, LCP 3.308 s, TBT 17 ms, CLS 0.00004, Speed Index 2.032 s, and TTI 3.308 s; every desktop run scored 100/100/100/100, with medians of FCP 0.445 s, LCP 0.686 s, TBT 0 ms, CLS 0.0149, Speed Index 0.529 s, and TTI 0.686 s. Browser/Poppler review verified the authored impact response, one-click resume after the impact pause, arbitrary scene-bound interactions, desktop/768 px/390 px/320 px reflow, portable scene colors, and the clean four-page PDF with its citation and reviewer handoff key. The clean-tree verifier intentionally fails because the dirty local working tree is uncommitted; freezing one commit and rerunning the gate in CI remains required.

**Earlier native browser evidence:** the operator-directed Chrome `ModelContext` trace belongs to the pre-polish 45-payload `/replay-sol/` artifact at 5,229,846 bytes with manifest SHA-256 `356be07e17a995608cfd558c685ba1fc9bf582b2f2fd530a9644604a8f2bd6ee`. It remains historical main-world constructor and mutation/human-gate evidence for that earlier artifact. The current exact-artifact bridge trace above did not inspect the constructor or exercise a mutation. The current public deployment remains `2855f0bc`, so neither trace is clean-commit CI, exact-candidate deployment, formal supported-model choice on that deployment, **Recently used/Sources**, or video evidence.
...
A separate uncoached GPT-5.6 Sol run exercised the current local seed-v6 source through the Codex in-app browser. It independently chose seven relevant read, validation, focus, preview, and audit calls; preserved the three unresolved questions, calibration advisory, and synthetic-evidence labels; opened a neutral cited preview; and never confirmed a claim, applied a proposal, changed canonical case version 1, or finalized the report. This is strong current-source product evidence, but it remains explicitly non-scoring until repeated against the exact deployed commit in ChatGPT desktop's native Site Tools client with its native trace captured.
