...
Application commit `2855f0bc50da2916128b2278a46f0d0a8a4e2bbd` passed GitHub Actions run `33184281134`, including verify job `98893121004` and deploy job `98894240126`, with **196/196 Vitest tests across 20 files** and **114 Playwright project runs: 109 passed, 5 intentional skips, and 0 failed**. Pages deployment `6143728209` published artifact `9691136611` (3,032,328 compressed bytes; SHA-256 `27c2bf89662de9280ddca52f9d2cb922545a913a27f819b855110f184e924da9`). An independent 2026-08-29 post-deploy comparison downloaded that artifact and cache-busted all **43 public files / 4,248,606 bytes**; every byte matched, yielding manifest SHA-256 `6eb13acc1eec75d60298a3979009a175e2ba94bc8e9ad00382d4a274bdcc6ba4`. These are clean CI, artifact-publication, and independent exact-payload comparison results; they do not by themselves prove a product journey or model behavior. This deployed commit predates the new automated `release-evidence.json` and `verify-deployment` workflow gate, and the live evidence endpoint is absent.

The fresh guide/scene/path browser smoke, 18-tool Codex discovery, and cache-busted Lighthouse 100/100/100/100 audit were recorded against superseded seed-v3 commit `00688d8a51fb783dbf147e08ece60470b8877544`. They remain historical evidence for that exact artifact and are not attributed to `2855f0bc`, the later seed-v6 working tree, a supported-model execution trace, or declarative-form verification.

Therefore the repository still does not claim a current supported-model pass rate, exact-candidate deployment identity, exact-deployment native tool execution, broad current-client compatibility, native declarative activation/cancel, cross-browser/screen-reader conformance, broad downloaded-export fidelity, or production response-policy deployment. The current dirty local 5,295,823-byte `1b4063e0d...` candidate passed the automated 12/12 artifact matrix and an operator-directed Codex in-app-browser trace of four native bridge calls. Summary, scene/questions state, all-scope validation, and question focus all returned `ok: true` at v1; the UI focus and four **No case change · observed v1** session entries agreed, with no browser warning or error. A finalized PDF downloaded by the exact automated journey was also parsed and rendered across all four pages without clipping or broken glyphs. That trace did not inspect a main-world `document.modelContext` constructor, exercise a mutation/lifecycle, establish model choice, capture **Recently used/Sources**, or prove deployment/cross-client behavior. GitHub Pages ignores `_headers` and shares its origin; a dedicated header-capable origin remains necessary for response-policy claims. The public YouTube deliverable is also outstanding.
