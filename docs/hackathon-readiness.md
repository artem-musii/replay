...
This document is an audit and submission plan. It does **not** assert that the final video has been recorded or that the supported-model run has passed. Application commit `cd88755b9b72e2e0a360a8a877584dc36c7c2053` has passed clean CI, deployed to the public URL, and been byte-verified there.
...
An operator also opened that exact release payload in the Codex in-app browser and invoked summary, scene/questions state, all-scope validation, and question focus through the native Site Tools bridge. All four calls returned `ok: true` at case v1; validation returned only `integrity.calibration-source`, focus visibly opened it, four session-only activity entries each said **No case change · observed v1**, and browser logs had no warning or error. This is exact-release-payload native bridge/UI-agreement evidence, not supported-model choice, **Recently used/Sources**, mutation/lifecycle, or broad cross-client evidence.
...
Application commit `cd88755b9b72e2e0a360a8a877584dc36c7c2053` passed Actions run `33269347192`: verify/build job `99144899873`, deploy job `99145725110`, and deployed-byte verification job `99145744458`. Pages deployment `6159028555` published artifact `9719700894`; the live `release-evidence.json` and an independent verifier matched all 46 payload files / 5,295,872 bytes with manifest SHA-256 `70323dbd1cd355dd3415a242e6c58a361d8617e35dd84a5cf3b1bc161b8e4e5c`. A post-deploy browser smoke reproduced the authored collision response, one-click resume after the one-time impact pause, the labelled high-speed case, visible 18-tool inventory, successful read/validation/activity Site Tools calls, and zero warning/error logs. This closes the release-identity blocker; it does not replace the supported-model or video gates above.
