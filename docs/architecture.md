...
Status: deployed seed-v6 architecture, reconciled with the repository on 2026-08-31. The calibrated-scene, motion-envelope, integrity, attestation, exact branch/provenance indexing, complete review-only start-to-final proposal, playback-session, and four-scenario work described here is implemented and deterministically tested. The live [`release-evidence.json`](https://artem-musii.github.io/replay/release-evidence.json) is authoritative for the current byte-verified commit and payload. It has not yet been exercised by a supported Site Tools model against the public deployment.
